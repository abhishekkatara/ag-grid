...
export class DashboardComponent implements OnInit {
  ...
  fetchLatestStatus() : void {
    this.http.get(`http://localhost:3000/api/v1/github/integration/status/${'abhishekkatara'}`).subscribe(
      (response: any) => {
        let integrationIndex = this.integrations.findIndex((_integration) => { return _integration.username == 'abhishekkatara'});
        this.integrations[integrationIndex].isSyncing = response.status;
      },
      (error) => {
        console.error('Failed to fetch integrations:', error);
        this.loading = false;
