...
const fetchLatestStatus = async (request, response) => {
  const { username } = request.params;
    const integration = await getModel('githubintegrations').findOne({ username, isDeleted: false });
    if (!integration) {
        response.status(404).send('Integration not found.');
        return;
    }
  response.json({ status: integration.isSyncing });
};
    ...
    try {
        ...
        for (const org of organizations) {
            ...
            for (const repo of repos) {
                ...
                await getModel('repos').updateOne(
...
const deleteStoreData = async (username) => {
  try {
      await getModel('organizations').updateMany(
        { identity: username },
        { $set: { isDeleted: true } }
      );
      await getModel('repos').updateMany(
        { identity: username },
        { $set: { isDeleted: true } }
      );
      await getModel('commits').updateMany(
        { identity: username },
        { $set: { isDeleted: true } }
      );
      await getModel('pulls').updateMany(
        { identity: username },
        { $set: { isDeleted: true } }
      );
      await getModel('issues').updateMany(
        { identity: username },
        { $set: { isDeleted: true } }
      );
